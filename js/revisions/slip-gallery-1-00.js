    ...
    function process(_sliders,settings)
    {
        if (settings && _sliders && _sliders.length > 0)
        {
            ...
            _sliders.each(function(i){
                ...
                //main slider slides - mainChildSelector is required
                if (settings.mainChildSelector)
                {
                    _sliderChildren = _slider.find(settings.mainChildSelector);
                    ...
                    if (_sliderChildren.length > 0)
                    {
                        ...
                        if (thumbsReady == false || (_sliderChildren.length == _thumbsChildren.length))
                        {
                            ...
                            _sentinelSlide = $(outerHTML(_firstSlide));
                            ...
                            controllers = {
                                _slider: _slider,
                                _sliderChildren: _sliderChildren,
                                _thumbsContainer: _thumbsContainer,
                                _thumbsWrap: _thumbsWrap,
                                _thumbsChildren: _thumbsChildren,
                                _sentinelSlide: _sentinelSlide,
                                _prevBtn: _prevBtn,
                                _nextBtn: _nextBtn,
                                newSlideIndex: false,
                                curSlideIndex: false,
    ...
    function scrollThumbsTo(controllers,settings)
    {
        ...
        if (controllers.newSlideIndex >= 0 && controllers.curSlideIndex >= 0 && controllers._thumbsContainer && controllers._thumbsWrap && controllers._thumbsChildren)
        {
            ...
            if (controllers._thumbsChildren.length > thumbsVisible)
            {
                ...
                if (maxThumbIndex > 0)
                {
                    var newSlideIndexC = (controllers.newSlideIndex > maxThumbIndex) ? maxThumbIndex : controllers.newSlideIndex;
                    var curSlideIndexC = (controllers.curSlideIndex > maxThumbIndex) ? maxThumbIndex : controllers.curSlideIndex;

                    var _newThumb = controllers._thumbsChildren.eq(newSlideIndexC);
                    var _curThumb = controllers._thumbsChildren.eq(curSlideIndexC);

                    var newThumbPos = _newThumb.position();
                    var curThumbPos = _curThumb.position();

                    var newLeft = newThumbPos.left;
                    var curLeft = curThumbPos.left;
                    var leftVal = '+=0px';

                    if (curLeft < newLeft)
                    {
                        leftVal = '-='+Math.abs(curLeft-newLeft)+'px';
                    }
                    else if (newLeft < curLeft)
                    {
                        leftVal = '+='+Math.abs(newLeft-curLeft)+'px';
                    }

                    controllers._thumbsWrap.stop(true,true).animate({
                        'left':leftVal
                    },500);
                }
            }
